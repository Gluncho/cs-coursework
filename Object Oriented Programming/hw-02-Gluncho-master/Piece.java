...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		...
		for(TPoint tp : points){
			min_x = Math.min(min_x, tp.x);
			min_y = Math.min(min_y, tp.y);
			max_x = Math.max(max_x, tp.x);
			max_y = Math.max(max_y, tp.y);
		}
		this.width = max_x - min_x + 1;
		this.height = max_y - min_y + 1;
		this.skirt = new int[this.width];
		for(int i = 0; i < this.width; i++){
			int min_y_on_column = width;
			for(TPoint tp : points){
				if(tp.x == i && min_y_on_column > tp.y){
					min_y_on_column = tp.y;
				}
			}
			this.skirt[i] = min_y_on_column;
		}
	}
	...
	*/
	...
	public Piece computeNextRotation() {
		...
		for(int i = 0; i < body.length; i++){
			int rotatedX = this.height - 1 - body[i].y;
			int rotatedY = body[i].x;
			rotatedPoints[i] =  new TPoint(rotatedX, rotatedY);
		}
	...
	*/
	public boolean equals(Object obj) {
		...
		List<TPoint> this_body = Arrays.asList(this.getBody());
		List<TPoint> that_body = Arrays.asList(other.getBody());
		return this_body.containsAll(that_body) && that_body.containsAll(this_body);
	}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		Piece current = root;
		while(true){
			Piece next = current.computeNextRotation();
			current.next = next;
			if(next.equals(root)) {
				current.next = root;
				break;
