...
class imdb {
 ...
 private:
  ...
  char* myBSearch(const char* key, const void* start,int (*cmpfn)(const char* c1, const char* c2)) const;
  void addFilms(int sz, char* res, vector<film>& films) const;
  void addPlayers(short sz, char* res,vector<string>& players) const;
