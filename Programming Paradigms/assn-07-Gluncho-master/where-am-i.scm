...
(define (partition pivot num-list cmp-fn)
  (if (null? num-list) '(() ())
      (let ((split-of-rest (partition pivot (cdr num-list) cmp-fn)))
	(if (cmp-fn (car num-list) pivot)
...
(define (quicksort num-list cmp-fn)
  (if (<= (length num-list) 1) num-list
      (let ((split (partition (car num-list) (cdr num-list) cmp-fn)))
	(append (quicksort (car split) cmp-fn) 
		(list (car num-list)) 
		(quicksort (cadr split) cmp-fn)))))
...
;; PROBLEM 1
(define (intersection-points lst)
  (if (or (null? lst) (null? (cdr lst))) '()
    (append (apply append(map (lambda(elem) (intersect (car lst) elem)) (cdr lst)))
    (intersection-points (cdr lst)))
  )
)

;; PROBLEM 2

(define (distance-product point lst)
  (apply * (map (lambda(elem) (dist elem point)) (remove point lst)))
)

;; PROBLEM 3
(define (rate-points lst)
  (map (lambda(elem) (list (distance-product elem lst) elem)) lst)
)

;; PROBLEM 4

(define (rate-point-cmp-fn rate-point1 rate-point2)
  (< (car rate-point1) (car rate-point2))
)

(define (sort-points lst)
  (quicksort lst rate-point-cmp-fn)
)

;; PROBLEM 5

(define (clumped-points lst)
  (prefix-of-list (map 
                    (lambda(elem) (cadr elem)) (sort-points (rate-points lst))) (floor (/ (length (sort-points (rate-points lst))) 2)))
)

;; PROBLEM 6

(define (average-point lst)
  ( let ((x (list (/ (apply + (map (lambda(elem) (car elem) ) lst)) (length lst) ) 
            (/ (apply + (map (lambda(elem) (cadr elem) ) lst)) (length lst) ))))
    (list (distance-product x lst) x)
  )
)

;; PROBLEM 7

(define (best-estimate lst)
  (
    average-point (clumped-points (intersection-points lst))
  )
)
...
(define (where-am-i distances stars)
  ( quicksort (map best-estimate (all-guesses distances stars)) rate-point-cmp-fn)
)
;;; --------------------------------------------------------------------------------------
;;; 				!!!!!!! TESTING PART !!!!!!!
