...
void generateText(map<string, Definition> *grammar,Definition* d);
...
int main(int argc, char *argv[])
{
  ...
  for(int i=1; i<=3; i++){
    cout<<"Version #"<<i<<": -------------"<<endl;
    Definition d=grammar["<start>"];
    generateText(&grammar,&d);
    cout<<endl;
  }
  return 0;
}
...
void generateText(map<string, Definition> *grammar, Definition* d){
  ...
  for(vector<string>::iterator it=p.begin(); it!=p.end(); ++it)
    if((*it).front()=='<' && (*it).back()=='>') {
      assert((*grammar).find(*it)!=(*grammar).end());
      generateText(grammar,&(*grammar)[*it]);
    }
    else cout<<(*it)<<" ";
}