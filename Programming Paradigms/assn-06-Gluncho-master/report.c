...
{
  sem_wait(&bank->report_lock); // whole method must be atomic
  ...
  if (r >= MAX_NUM_REPORTS) {
      sem_post(&bank->report_lock); // if we dont do this, other transfers will be locked permanently
...
Report_DoReport(Bank *bank, int workerNum)
{
  ...
  assert(rpt); Y;
  bank->num_done++;
  if(bank->num_done != bank->num_workers){
    sem_post(&bank->check);
    sem_wait(&bank->isReady_account[workerNum]);
    return 0;
  }
  if (rpt->numReports >= MAX_NUM_REPORTS) {
      ...
      for(int i = 0; i < bank->num_workers; i++){
        if(i == workerNum) continue;
        sem_post(&bank->isReady_account[i]);
      }
      sem_post(&bank->check);
      return -1;
  }
